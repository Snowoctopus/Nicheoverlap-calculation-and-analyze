...
#                                                                                                           #
# inputs:                                                                                                   #
#        - trab: a matrix (x, N+1) with the abundances of the N species at x values of the considered trait  #
...
nicheoverlap<-function(trab,graph=T,colorsN=NA) {
  ...
  x<-resdens[,1]
  densN<-resdens[,2:(N+1)]
  #delete the # in the following two lines if you want to record the probability density of two species along the assigned gradient
  ...
  NO<-round(overlap,3)
  #delete the # in the following two lines if you want to record the niche overlap area of the assigned type of microbes
  options(max.print=1000000)
  print(NO)
  #############################################################################################################
  # graphic
  if (graph==T) {
    ...
    if (is.na(colorsN[1])==T) color<-c("#006400","#1C86EE")
    #, "#4B8AEF", "#11A123", "#9D939C", "#761EA1", "#966410"
    ...
    # plot of axes
    plot(mean(labx),mean(laby), type="n",xlim=range(x), ylim=range(laby), xlab="", ylab="", xaxt="n", yaxt="n",axes=F,family="mono")
    title(sub=paste("Niche overlap=",round(NO,3),sep=""),line=2,cex.sub=1.2 ,family="sans")
    axis(side=1, labx[labx<=max(x) & labx>=min(x)], tcl=-0.2, pos=0, labels=F,family="sans")
    mtext(labx[labx<=max(x) & labx>=min(x)], side=1, at=labx[labx<=max(x) & labx>=min(x)], las=1,line=-0.2, cex=1,family="sans")
    axis(side=2, laby, tcl=-0.2, pos=min(x),labels=F) ; mtext(laby, side=2, at=laby, las=1, line=-0.4, cex=1,family="sans")
    #record the peak position of nicheoverlap area_written by MG
    for (k in 1:N )
      test1<- rbind(c(min(x),0), c(min(x),minN[1]) , cbind(x,minN), c(max(x),0))
    #options(max.print=1000000)
    test1max_index<-which.max(test1[,2])
    test1max_depth<-test1[test1max_index,1]
    #delete the # in the following two lines if you want to record the quasi-asymmetry of niche overlap
    #options(max.print=1000000)
    #print(test1max_depth)
    #end the record
    #test1range_shallow_index<-which.max(test1[,2])[1]
    #test1range_shallow_depth<-test1[test1max_index,1]
    ########################
    # plot of densities
    for (k in 1:N )
